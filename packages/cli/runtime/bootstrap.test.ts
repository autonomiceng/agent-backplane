...
test("rerun overwrites secrets or repeats provisioning after an interrupted creation", async () => {
  ...
  try {
    ...
    } }) : args[0] === "image" ? `sha256:${"e".repeat(64)} amd64` : args[0] === "create" ? args.at(-1) === "--version" ? args.includes("/usr/bin/bun") ? "1.4.2" : "workerd 2026-09-18" : `${"d".repeat(64)}  /usr/bin/workerd\na83d263767d839e4d2649ca8e35d07159c7afc99afdc96d731ced29e056dda0c  /usr/bin/bun` : args[0] === "context" ? "unix:///var/run/docker.sock" : args[0] === "network" && args[1] === "inspect" ? '[{"Subnet":"172.30.0.0/24","IPRange":"172.30.0.128/25","Gateway":"172.30.0.1"}]' : args[0] === "volume" ? existingVolume ? "existing-data" : "" : args.at(-1) === "/data/enrollment/capability" ? secret : args.some(arg => arg.includes("curl")) ? JSON.stringify({ enrollment: { state: "pending" } }) : "";
    };
