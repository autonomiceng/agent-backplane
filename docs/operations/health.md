...
| `archive_unavailable`, `archive_stale` | Check `pg_stat_archiver`, the backup mount, free space and archive-command errors. The PostgreSQL user must own `archive/` in the repository; rerun `scripts/bootstrap.py` after mounting a new one. |
