...
| `archive_unavailable`, `archive_stale` | Check `pg_stat_archiver`, the backup mount, free space and archive-command errors. The `postgres` service assigns `archive/` to the PostgreSQL user when it starts; a root-squashed or read-only mount refuses that. |
...
server and proxied unchanged by the standalone edge or Platform Edge. It lists the components `server`,
