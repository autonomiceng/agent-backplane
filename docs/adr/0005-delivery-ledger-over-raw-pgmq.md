...
PGMQ stores and dispatches messages, but agents never call it. They use typed verbs: send (idempotency key required), claim, renew, ack, nack, hold, inspect, replay, cancel. Every claim returns a Receipt bound to Principal, Run, attempt and expiry, and every later transition must present a valid Receipt. Retry policy, backoff, maximum attempts and dead-lettering belong to the server. Queues are unordered in v1.
