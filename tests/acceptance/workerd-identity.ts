...
test("effective runtime identity preserves deployer authority and refuses a wrong expectation before registration", async () => {
  ...
  const artifact = await compute.verify(AbortSignal.timeout(5000));
  if (!artifact) throw Error("owned runtime identity verification failed");
  ...
  try {
    ...
    expect(await invoked.json()).toMatchObject({ deploymentId: input.id, result: { ok: true } });
    expect(await pool<{ principal_id: string; run_id: string; artifact: typeof artifact }[]>`SELECT principal_id, run_id, metadata->'artifact' AS artifact
      FROM audit.events WHERE workspace_id = ${workspaceId} AND kind = 'function.invoke'`).toEqual([{ principal_id: principalId, run_id: runId, artifact }]);
