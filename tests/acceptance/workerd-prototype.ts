...
import { createSupervisor } from "../../apps/server/compute/workerd/supervisor.ts";
...
const supervisor = createSupervisor({ binary, config: new URL("../../apps/server/compute/workerd/config.capnp", import.meta.url).pathname,
  timeoutMs: 10000, token: Bun.env.BP_COMPUTE_TOKEN ?? "", args: [`--external-addr=api=${Bun.env.BP_TEST_API_ADDRESS ?? "127.0.0.1:9"}`],
...
const server = Bun.serve({ hostname: "127.0.0.1", port: Number(Bun.env.BP_TEST_SUPERVISOR_PORT ?? 0), maxRequestBodySize: 27 * 1048576, fetch: supervisor.fetch });
