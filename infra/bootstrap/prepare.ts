...
export async function prepare(argv: string[], env: Environment, run: Runner = docker, recordStatus: StatusRecorder = statusRecorder): Promise<string> {
  ...
  try {
    ...
    if (source === undefined || output !== originalSource || additions.length) await privateWrite(path, output + (output && !output.endsWith("\n") ? "\n" : "") + (additions.length ? additions.join("\n") + "\n" : ""), source !== undefined);
