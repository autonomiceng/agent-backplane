...
export async function prepare(argv: string[], env: Environment, run: Runner = docker): Promise<string> {
  ...
  try {
    ...
    if (profiles.includes("blobs")) for (const key of ["BP_BLOB_BOOTSTRAP_IMAGE"]) {
      if (!/^[^\s@]+@sha256:[a-f0-9]{64}$/.test(entries[key] ?? "")) throw new CliError("blob_image_pin_required", 1);
    }
    ...
    if (source === undefined || additions.length) await privateWrite(path, (source ?? "") + (source && !source.endsWith("\n") ? "\n" : "") + additions.join("\n") + "\n", source !== undefined);
    ...
    await run([...compose, "up", "--wait"], child);
