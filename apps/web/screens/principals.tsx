...
export function PrincipalsView({ state, actions }: { state: PrincipalsState; actions: PrincipalsClient }): ReactNode {
  ...
  return <section>
    ...
    <table><thead><tr><th>Name</th><th>ID</th><th>Key prefix</th><th>First issued</th><th>Last use</th><th>Rotated</th><th>Status</th><th>Revoked</th><th>Action</th></tr></thead>
      <tbody>{state.items.map((principal) => <tr key={principal.id}>
        ...
        <td>{principal.credential?.createdAt ?? "No key issued"}</td>
        <td>{principal.credential ? principal.credential.lastUsedAt ?? "No use recorded for current key" : "No key issued"}</td>
        <td>{principal.credential?.rotatedAt ?? "Never rotated"}</td><td>{principal.status}</td><td>{principal.credential?.revokedAt ?? "—"}</td>
        <td><button disabled={pending || principal.status === "revoked"} onClick={() => actions.select(principal)}>Revoke {principal.name}</button></td>
      </tr>)}</tbody></table>
    ...
    {selected && <section aria-label="Principal revocation">
      ...
      {selected.phase === "confirming" && <>
        ...
        <button onClick={() => { void actions.confirm(); }}>Confirm revocation</button>
