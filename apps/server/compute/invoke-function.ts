...
export async function invokeFunction(pool: Pool, caller: RunContext, name: string, body: typeof invokeFunctionInput.static, launcher: ComputeLauncher, signal: AbortSignal) {
  ...
  const artifact = await launcher.verify(AbortSignal.any([signal, AbortSignal.timeout(2000)]));
  if (!artifact) throw new InvocationError("compute_unavailable");
  const invocation = await withRunContext(pool, caller, async (tx, emit) => {
    ...
    await emit("function.invoke", [name, active.id, child.runId], 1, { deploymentId: active.id, runId: child.runId, artifact });
