...
export async function rebuildMigrationProjection(pool: Pool, context: Extract<RunContext, { userId: string }>, projection?: MigrationProjection): Promise<ProjectionResult> {
  try {
    ...
    return projection ? await projection.project(context) : { ok: false, status: 503, error: "projection_unavailable" };
  } catch (error) {
