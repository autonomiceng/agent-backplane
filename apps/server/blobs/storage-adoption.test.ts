...
import { adoptionFixture, reconciliation, initialization } from "./testing/storage-adoption-fixture.ts";
...
test("an unbound store with data is never bound: initialization and reconciliation refuse before writing intent", async () => {
  const f = await adoptionFixture();
  try {
    await f.writeBlob();
    await expect(f.operate(initialization)).rejects.toThrow("blob_binding_unbound_data_unsupported");
    await expect(f.operate({ ...reconciliation, fenced: false })).rejects.toThrow("blob_binding_checkpoint_and_fence_required");
    await expect(f.operate(reconciliation)).rejects.toThrow("blob_binding_intent_mismatch");
    await expect(f.operate({ ...reconciliation, retain: true })).rejects.toThrow("blob_binding_intent_mismatch");
    ...
    await expect(f.verify()).rejects.toThrow("blob_binding_required");
  } finally { await f.close(); }
});
...
test("publication failures retry only their exact durable intent and inventory", async () => {
  for (const published of [false, true]) {
    ...
    try {
      ...
      } }, initialization)).rejects.toThrow("injected_publication_disconnect");
      const [intent] = await f.admin`SELECT * FROM control.blob_storage_binding`;
      expect(intent.phase).toBe("verifying");
      expect(await f.operate({ ...reconciliation, mode: "inspect", checkpoint: "" })).toMatchObject({
        ...
        intent: { phase: "verifying", operation: "initialize", checkpoint: "", retainUnreferenced: false },
      });
      expect(Boolean(await f.store.markerOrAbsent())).toBe(published);
      await expect(f.verify()).rejects.toThrow("blob_binding_not_ready");
      await expect(f.operate({ ...initialization, retain: true })).rejects.toThrow("blob_binding_intent_mismatch");
      await expect(f.operate(reconciliation)).rejects.toThrow("blob_binding_intent_mismatch");
      const stray = crypto.randomUUID();
      await f.store.stage(stray, stray, Buffer.from("stray"));
      await expect(f.operate(initialization)).rejects.toThrow("blob_binding_unbound_data_unsupported");
      await rm(join(f.dataDir, "blobs", stray), { recursive: true });
      expect((await f.operate(initialization)).status).toBe("ready");
...
test("explicit retention preserves staged and unreferenced bytes through cleanup and reconciliation", async () => {
  ...
  try {
    expect((await f.operate(initialization)).status).toBe("ready");
    const blob = await f.writeBlob(), extra = crypto.randomUUID(), bytes = Buffer.from("leftover");
    await f.verify();
    ...
    const inspection = await f.operate({ ...reconciliation, mode: "inspect", checkpoint: "" });
    expect("binding" in inspection && inspection.binding?.phase).toBe("ready");
    expect("objects" in inspection && inspection.objects.filter(ref => ref.classification === "unreferenced")).toHaveLength(2);
    await expect(f.operate(reconciliation)).rejects.toThrow("blob_binding_unreferenced_requires_retention");
    expect((await f.operate({ ...reconciliation, retain: true })).status).toBe("ready");
    await f.verify();
    ...
    await expect(f.verify()).rejects.toThrow("blob_binding_inventory_mismatch");
    expect((await f.operate({ ...reconciliation, checkpoint: "checkpoint-2", retain: true })).status).toBe("ready");
    await f.verify();
