...
import { releaseInput, releaseParams } from "./release-input.ts";
import { release } from "./release.ts";
import { queueErrorStatus } from "./queue-error.ts";

export function releaseRoute(pool: Pool, auth: Auth, authUrl: string) {
  return new Elysia({ name: "release" }).use(userSession(auth, authUrl)).post(
    "/api/v1/workspaces/:workspaceId/deliveries/:deliveryId/release",
    async ({ user, params, status, set }) => {
      set.headers["Cache-Control"] = "no-store";
      const result = await release(pool, { workspaceId: params.workspaceId, userId: user.id }, params.deliveryId);
    ...
    },
    {
      ...
      transform({ body, request, status }) {
        ...
        if (typeof body !== "object" || body === null || Array.isArray(body)
          || Object.keys(body).some((key) => !Object.hasOwn(releaseInput.properties, key))) {
      ...
      detail: { "x-backplane-auth": "user", "x-backplane-run": "none", operationId: "releaseDelivery", tags: ["queue"] },
    },
  );
}
