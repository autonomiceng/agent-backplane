...
export async function probeReadiness(pool: Pool, expectedSchemaVersion: number, enrollment?: Enrollment): Promise<Readiness> {
  try {
    const facts = await clusterFacts(pool, enrollment);
...
function clusterFacts(pool: Pool, enrollment?: Enrollment): Promise<ClusterFacts> {
  return probeTransaction(pool, PROBE_TIMEOUT_MS, async (tx) => {
    ...
    return {
      ...(enrollment ? { enrollment: (await enrollment.observe(tx)).state, signup: enrollment.signup, publicSignup: enrollment.publicSignup } : {}),
      restoreGate,
